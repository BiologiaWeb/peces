...
async function enterApp() {
  ...
  // Filtro de acciones en bitácora: ocultar sup-only para operadores
  document.querySelectorAll('.log-filter-sup-only').forEach(el => {
    el.style.display = canManage() ? '' : 'none';
  });

  // Cargar usuarios en el filtro de responsable (solo sup/admin)
...
// Carga todos los usuarios en el combobox de filtro (solo sup/admin)
...
window.downloadLogCSV = () => {
  ...
  // Builds a plain-text detail string for the CSV (no HTML, no "Notas:" prefix)

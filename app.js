...
let _currentLogRows = [];
let _supervisors    = [];
let _exps           = [];            // expedientes
let _params         = [];            // parámetros bitácora
let _sesionParam    = null;          // sesión de medición activa
let _piletaQueue    = [];            // cola registro rápido
let _piletaQueueIdx = 0;
let _paramAreasDyn  = null;          // áreas personalizadas (Firestore)
let _detPondId = null, _detPondName = null;
let _detFishId   = null;  // fauna abierta en modal
let _detailExpId = null;  // expediente abierto en modal
...
const canEditPond  = () => canManage() && !isMortArea();  // solo sup/admin no-mortalidad edita estanques
const canDoActions = () => !isMortArea() && (isAdmin() || isSupervisor() || isOperator()); // operador normal sí puede registrar acciones básicas
...
function _scope(items) {
  ...
  // Operador: ve todo lo de su área
  // (el supervisorId es solo para saber quién es su jefe, no para filtrar estanques)
  if (area) return items.filter(i => i.areaId === area);
  // Fallback legacy: sin área, filtra por supervisorId
...
window.doLogout = () => {
  try { _clearSession(); } catch {}
  try { _pondsCache = []; _fishCache = []; _currentLogRows = []; } catch {}
  try { _supervisors = []; _exps = []; _params = []; } catch {}
  try { _sesionParam = null; _piletaQueue = []; _piletaQueueIdx = 0; _paramAreasDyn = null; } catch {}
  try { clearInterval(window._logInterval); } catch {}
  try {
    ['pond-grid','fish-list','log-tbody','users-tbody','exp-tbody','params-tbody'].forEach(id => {
...
async function _enterApp() {
  ...
  if ($('tab-users')) $('tab-users').style.display = (isAdmin() || isSupervisor()) ? 'block' : 'none';
...
function _fillPondSupSel() {
  const sel = $('pond-supervisor'); if (!sel) return;
  sel.innerHTML = '<option value="">— Sin asignar —</option>' +
    _supervisors.map(s => `<option value="${s.uid}">@${s.username} — ${s.name}${s.areaId?' ('+(_areaLabel(s.areaId))+')':''}</option>`).join('');
}
...
window.openAddUser = () => {
  ...
  // Supervisor: pre-seleccionar su área (no la cambia)
  const areaEl = $('new-user-area');
  if (isSupervisor() && areaEl) {
    areaEl.value = myArea() || '';
    const areaGrp = $('new-user-area-group');
    if (areaGrp) areaGrp.style.display = 'none'; // supervisor no necesita elegir área
  }
  onNewUserRoleChange();
...
async function _loadUsers() {
  ...
  // Supervisor: solo ve sus operadores (no otros supervisores ni admins)
  if (isSupervisor() && !isAdmin()) {
    users = users.filter(u => u.supervisorId === _session.uid);
  }
  ...
  $('users-tbody').innerHTML = users.map(u => `
    <tr>
      ...
      <td><div style="display:flex;gap:.4rem;flex-wrap:wrap">
        ...
        ${isAdmin() && u.role === 'operador' ? `<button class="btn btn-ghost btn-sm" onclick="openReassignSupervisor('${u.uid}','${u.username}','${u.supervisorId||''}','${u.areaId||''}')">Supervisor</button>` : ''}
...
window.openAddPond = () => {
  ...
  const supGrp = $('pond-supervisor-group');
  if (supGrp) { supGrp.style.display = isAdmin() ? 'block' : 'none'; if (isAdmin()) _fillPondSupSel(); }
...
window.openEditPond = id => {
  ...
  const supGrp = $('pond-supervisor-group');
  if (supGrp) {
    supGrp.style.display = isAdmin() ? 'block' : 'none';
    if (isAdmin()) {
      _fillPondSupSel();
      setTimeout(() => {
        const sel = $('pond-supervisor');
        if (sel && p.supervisorId)
          for (let i = 0; i < sel.options.length; i++)
            if (sel.options[i].value === p.supervisorId) { sel.selectedIndex = i; break; }
      }, 30);
    }
  }
  _open('modal-add-pond');
...
window.savePond = async () => {
  ...
  try {
    if (editId) {
      // Al editar: actualizar también supervisor y área si el admin los cambió
      if (isAdmin()) {
        const selSupId = $('pond-supervisor')?.value || null;
        const supData  = selSupId ? _supervisors.find(s => s.uid === selSupId) : null;
        data.supervisorId = selSupId || null;
        data.areaId       = supData?.areaId || null;
      }
      await updateDoc(doc(db, 'ponds', editId), data);
      _toast('Estanque actualizado');
    } else {
      // Al crear: supervisor = el logueado (si es sup) o el seleccionado (si es admin)
      let supId  = isSupervisor() ? _session.uid : ($('pond-supervisor')?.value || null);
      const supData = supId ? _supervisors.find(s => s.uid === supId) : null;
      const areaId  = supData?.areaId || myArea() || null;
      if (!areaId) { _toast('Selecciona el supervisor del estanque', 'err'); return; }
      await addDoc(collection(db, 'ponds'), {
        ...data, areaId, supervisorId: supId,
        createdBy: _name(), createdByUser: _user(), createdAt: serverTimestamp()
      });
...
window.openRegisterAction = pondId => {
  ...
  ['nacimiento','introducción','mortalidad'].forEach(val => {
    const opt = document.querySelector(`#action-type option[value="${val}"]`);
    if (!opt) return;
    if (val === 'mortalidad')    opt.style.display = (isMortSup() || isAdmin()) ? '' : 'none';
    else /* nac / intro */       opt.style.display = (canManage()  || isAdmin()) ? '' : 'none';
  });
  ...
  // isMortSup solo puede registrar mortalidad — seleccionarlo automáticamente
  $('action-type').value = isMortSup() ? 'mortalidad' : 'alimentación';
  onActionTypeChange();
...
window.saveAction = async () => {
  ...
  try {
    if (action === 'mortalidad') {
      if (!isMortSup() && !isAdmin()) { _toast('Solo el supervisor de Mortalidad puede registrar bajas', 'err'); return; }
    ...
    if (action === 'nacimiento') {
      if (!canManage() && !isAdmin()) { _toast('Solo supervisores pueden registrar nacimientos', 'err'); return; }
    ...
    if (action === 'introducción') {
      if (!canManage() && !isAdmin()) { _toast('Solo supervisores pueden registrar introducciones', 'err'); return; }
    ...
    ['intro-name','intro-species','intro-fish-notes','intro-qty-alevin','intro-qty-juvenil',
     'intro-qty-adulto','intro-qty-existing'].forEach(id => { const e=$(id); if(e) e.value=''; });
    ['intro-fish','intro-cat'].forEach(id => { const e=$(id); if(e) e.selectedIndex=0; });
    document.querySelectorAll('input[name="intro-type-radio"]').forEach((r,i) => r.checked = i===0);
    onIntroTypeChange();
    _toast('Introducción registrada'); await _loadFish(); return;
    }
...
async function _loadLog() {
  ...
  if (isAdmin()) {
    ...
    // supervisor: ve acciones de su área O donde él es supervisor (acciones viejas sin areaId)
    const myAreaId = _session?.areaId;
    rows = rows.filter(r =>
      (myAreaId && r.areaId === myAreaId) ||
      r.supervisorId === _session.uid
    );
  } else {
  ...
  if (from) {
    ...
    const d = new Date(fy, fm-1, fd, 0, 0, 0, 0);  // local midnight
    rows = rows.filter(r => (r.timestamp?.toDate?.() ?? new Date(r.timestamp)) >= d);
  }
  if (to) {
    const [ty,tm,td] = to.split('-').map(Number);
    const d = new Date(ty, tm-1, td, 23, 59, 59, 999);  // local end of day
    rows = rows.filter(r => (r.timestamp?.toDate?.() ?? new Date(r.timestamp)) <= d);
  }
...
async function _loadExps() {
  ...
  const fromEl = $('exp-filter-from'), toEl = $('exp-filter-to');
  if (fromEl && !fromEl.value) fromEl.value = _today();
  if (toEl   && !toEl.value)   toEl.value   = _today();
  ...
  // Si hay filtro activo, aplicarlo; si no, mostrar todos
  const hasFilter = (fromEl?.value || toEl?.value || $('exp-filter-q')?.value || $('exp-filter-area')?.value);
  if (hasFilter) filterExps(); else _renderExps(_exps);
}
...
window.downloadExpCSV = () => {
  ...
  const esc = v => `"${String(v ?? '').replace(/"/g, '""')}"`;
  // Micro: expandir 5 tubos x 4 campos
  const microHdr = [1,2,3,4,5].flatMap(i => [`Micro T${i} Tubo`,`Micro T${i} Caja`,`Micro T${i} Tinción`,`Micro T${i} Ident`]);
  const parHdr   = ['Par Tremátodos','Par Céstodos','Par Nemátodos','Par Acantocéfalos','Par Sanguijuelas','Par Copépodos','Par Fases juv','Par Estado','Par Larvas de','Par Estado larvas','Par Protozoarios','Par Fijador','Par Preservador'];
  const micHdr   = ['Mic Toma','Mic Colorante','Mic Observado'];
  const headers  = ['ID','Fecha','Especie','Cantidad','Área','Acuario','Colectó','Entregó',
                    ...
                    ...microHdr, ...parHdr, ...micHdr];
  const microRow = r => [1,2,3,4,5].flatMap(i => {
    const m = r.microbiologico?.[i] || {};
    return [m.tubo||'', m.caja||'', m.tincion||'', m.ident||''];
  });
  const parKeys  = ['trematodos','cestodos','nematodos','acanto','sangui','cope','fases-juv','estado1','larvas','estado2','protozoo','fijador','preservador'];
  const parRow   = r => parKeys.map(k => r.parasitologico?.[k] || '');
  const micRow   = r => ['toma','colorante','observado'].map(k => r.micologico?.[k] || '');
  const lines = [
...
const PARAM_AREAS = {
  conservacion_piletas: {
    ...
    sectores: {
      // 'Sector A': ['A1','A2','A3','A4','A5','A6'],  // No existe actualmente
...
function _renderParams(rows) {
  ...
  tb.innerHTML = rows.map(r => `
    <tr>
      ...
      ${(isMortSup()||isAdmin())?`<td style="width:40px"><button class="btn btn-ghost btn-sm" onclick="editParam('${r.id}',${r.ph??0},${r.temperatura??0},'${r.pileta||''}','${r.sector||''}')">✎</button></td>`:'<td></td>'}
    </tr>`).join('');
}
...
window.agregarParticipante = async () => {
  ...
  try {
    ...
    if (ud.areaId !== 'mortalidad') { errEl.textContent = 'Solo usuarios del área de Mortalidad'; errEl.style.display = 'block'; return; }
...
function _getParamAreas() {
  return _paramAreasDyn || PARAM_AREAS;
}
...
async function _loadParamAreasConfig() {
  try {
    ...
    if (!snap.empty) {
      ...
      if (d.areas) _paramAreasDyn = d.areas;
    }
  } catch {}
}
...
async function _saveParamAreasConfig() {
  try {
    const snap = await getDocs(query(collection(db, 'param_config')));
    const data = { areas: _paramAreasDyn || PARAM_AREAS, updatedAt: serverTimestamp(), updatedBy: _user() };
...
window.editParam = (id, ph, temp, pileta, sector) => {
  // Abrir modal pequeño para corregir ph y temperatura
  const nPh   = prompt(`Corregir pH de ${pileta}${sector?' ('+sector+')':''}:`, ph);
  if (nPh === null) return;
  const nTemp = prompt('Corregir temperatura (°C):', temp);
  if (nTemp === null) return;
  if (!nPh || !nTemp) { _toast('Valores inválidos', 'err'); return; }
  updateDoc(doc(db, 'parametros', id), { ph: parseFloat(nPh), temperatura: parseFloat(nTemp) })
    .then(() => { _toast('Parámetro corregido'); _loadParams(); })
    .catch(() => _toast('Error al actualizar', 'err'));
};

...
//   Configuración Firebase — reemplaza si cambias proyecto
...
// Cache de fauna para el modal de detalle (evita pasar JSON en onclick)
let fishCache = [];
  ...
  document.getElementById('fish-detail-body').innerHTML = `
    <div class="fish-detail-wrap">
      ...
      <div class="fish-detail-grid">
        ...
        ${f.notes ? `<div class="fd-row"><span class="fd-label">Notas / Descripción</span><span class="fd-value">${f.notes}</span></div>` : ''}
        <div class="fd-row"><span class="fd-label">Registrado por</span><span class="fd-value">@${f.addedByUser || f.addedBy || '—'}</span></div>
        ${f.addedAt ? `<div class="fd-row"><span class="fd-label">Fecha de registro</span><span class="fd-value">${fmtDate(f.addedAt)}</span></div>` : ''}
      </div>
    </div>`;
...
window.loadFishView = async () => {
  ...
  c.innerHTML = items.map(f => `
    <div class="fish-row" onclick="openFishDetail('${f.id}')" style="cursor:pointer">
      <div class="fish-info">
        <div class="fname">${f.name} <span style="color:var(--text-lt);font-size:.72rem;font-weight:600">· ${f.pondName}</span></div>
        <div class="fspec">${f.species || 'Especie no especificada'} &nbsp;·&nbsp; @${f.addedByUser || f.addedBy}</div>
      </div>
